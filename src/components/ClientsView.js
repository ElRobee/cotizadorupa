...
const QuotationsView = ({
  ...
  onSearchChange,
  filters,
  setFilters, // Make sure setFilters is passed as a prop from the parent component
  showFilters,
  setShowFilters,
  ...
  const clearFilters = () => {
    ...
    // Also clear the search term if needed
    if (onSearchChange) {
  ...
  return (
    <div className={`flex-1 p-8 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <Filters
        ...
        clearFilters={clearFilters} // Pass the new function
        theme={theme}
        darkMode={darkMode}
      />
    </div>
